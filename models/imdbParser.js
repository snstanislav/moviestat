...
(data, imdbId, imdbLink, pRating, pDateTime)=> {
  ...
  let imdbRating = $(`a[href*="ratings/?ref_=tt_ov_rt"] div[data-testid="hero-rating-bar__aggregate-rating__score"] >
    span`).text();
  imdbRating = imdbRating.substring(0, imdbRating.indexOf("/"))

  let imdbRatingNum = $($(`a[href*="ratings/?ref_=tt_ov_rt"] > span > div div
    div:last-child`)[0]).text().trim();
  ...
  let director = Array.from($(`p[data-testid="plot"] + div > div > ul >
    li[data-testid="title-pc-principal-credit"] span:Contains("Director") + div > ul > li a`));
